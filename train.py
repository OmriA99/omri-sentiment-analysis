...
def train(args):
    ...
    if DYN_RNN_COPY_THROUGH_STATE:
        _, last_state = tf.nn.dynamic_rnn(stacked_rnn_cell, data, dtype=tf.float32, sequence_length=input_data_lengths)
    else:
        _, last_state = tf.nn.dynamic_rnn(stacked_rnn_cell, data, dtype=tf.float32)
    ...
    # state is a LSTMStateTuple that contains state's c,h
